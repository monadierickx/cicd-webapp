...
app.get('/animals/random', asyncMiddleware(async (req, res) => {
  const random_id = Math.floor(Math.random() * 10) + 1;
  const random_animal = await persistence.getAnimal(Number(random_id));
  res.json(random_animal);
}));

app.get('/animals/:id', asyncMiddleware(async (req, res) => {
  const animal = await persistence.getAnimal(Number(req.params.id));
  // console.log("random logmessage")
  res.json(animal);
}));
